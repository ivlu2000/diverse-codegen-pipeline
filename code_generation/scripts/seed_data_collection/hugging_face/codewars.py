...
ds = load_dataset("NyanDoggo/codewars_dataset")

# Filter out those tasks that have the category bug_fixes
ds = ds.filter(lambda x: x["category"] != "bug_fixes")


# Extract unique tasks
tasks = list(set(ds["train"]["description"]))

print("Size of Codewars dataset: ", len(tasks))
